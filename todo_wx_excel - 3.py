...
def calc_todo_progress(path) -> int:
    ...
    for ws in wb.worksheets:
        for row in ws.iter_rows(min_row=2, values_only=True):
            task, status, note, tanggal = row[:4]
...
class ItemDialog(wx.Dialog):
    """Dialog tambah/edit task: task + note + tanggal dalam 1 submit."""
...
class TodoApp(wx.Frame):
    def __init__(self):
        ...
        bar.Add(self.sheet_list, 0, wx.RIGHT, 12)
        ...
        bar.Add(btn_add_sec, 0, wx.RIGHT, 6)
        bar.Add(btn_ren_sec, 0, wx.RIGHT, 6)
        bar.Add(btn_del_sec, 0, wx.RIGHT, 12)

        bar.Add(wx.StaticText(main_panel, label="Filter Tanggal:"), 0, wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, 6)
        self.date_filter = wx.Choice(main_panel, choices=["Semua Tanggal"])
        self.date_filter.Bind(wx.EVT_CHOICE, lambda e: self.show_tasks())
        bar.Add(self.date_filter, 0)
    ...
    def create_todo(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if os.path.exists(path):
                wx.MessageBox("Nama sudah ada", "Error")
                return
            create_new_todo_excel(path)
    ...
    def rename_todo(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if self.current_file == old_path:
                ...
                self.lbl_title.SetLabel(f"# {new_name}")
    ...
    def on_select_todo(self, event):
        ...
        self.lbl_title.SetLabel(f"# {name}")
        self.load_sheets()
    ...
    def load_sheets(self):
        ...
        self.sheet_list.SetSelection(0)
        self.show_tasks()
    ...
    def delete_section(self, event):
        ...
        if len(wb.sheetnames) <= 1:
            wx.MessageBox("Minimal 1 section", "Info"); return
        ws = wb[self.current_sheet]
        wb.remove(ws); wb.save(self.current_file)
        self.current_sheet = None
        self.load_sheets()

    # === CRUD Task
    def clear_tasks(self):
        for c in self.task_area.GetChildren(): c.Destroy()
        self.task_sizer.Layout()
        self.scroll.Layout()

    def refresh_date_filter(self, ws):
        dates = set()
        for row in ws.iter_rows(min_row=2, values_only=True):
            if row and row[3]:
                d = str(row[3])[:7]  # YYYY-MM
                dates.add(d)
        items = ["Semua Tanggal"] + sorted(dates)
        self.date_filter.Set(items)
    ...
    def show_tasks(self):
        ...
        ws = wb[self.current_sheet]
        if ws.max_row == 0:
            ws.append(HEADER); wb.save(self.current_file)

        # isi filter tanggal
        self.refresh_date_filter(ws)
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            # filter
            if selected_filter and selected_filter != "Semua Tanggal":
                if not str(tanggal).startswith(selected_filter): continue
            ...
            cb = wx.CheckBox(row_panel, label=f"{task} [{tanggal}]")
    ...
    def toggle_task(self, event, task_name):
        ...
        for row in ws.iter_rows(min_row=2):
            if str(row[0].value) == task_name:
                row[1].value = 1 if event.IsChecked() else 0
                break
        wb.save(self.current_file)
        self.show_tasks()
